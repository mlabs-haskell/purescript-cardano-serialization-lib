...
import Cardano.Serialization.Lib (PlutusList, bigInt_fromStr, bigNum_fromStr, constrPlutusData_new, cslToAesonViaBytes, hashPlutusData, packListContainer, plutusData_newConstrPlutusData, plutusData_newInteger, plutusList_new, transactionWitnessSet_new, transactionWitnessSet_setPlutusData)
...
legacyOutputRoundtrip :: forall a. Bind a => MonadEffect a => MonadThrow Error a => a Unit
legacyOutputRoundtrip = do
  plutusList <- liftEffect plutusList_new
  let
    constr0 = plutusData_newConstrPlutusData $ constrPlutusData_new (nonNull $ bigNum_fromStr "0") plutusList
    constr0Hash = encodeAeson $ hashPlutusData constr0
  (show constr0Hash) `shouldEqual` "\"923918e403bf43c34b4ef6b48eb2ee04babed17320d8d1b9ff9ad086e86f44ec\""

plutusListSerializationCliCompatibility :: forall a. Bind a => MonadEffect a => MonadThrow Error a => a Unit
...
spec = do
  describe "Plutus" do
    ...
    it "Plutus List Serialization CLI Compatibility" plutusListSerializationCliCompatibility
